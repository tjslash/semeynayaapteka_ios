...
class ListDrugsViewModel: DrugCellModelDelegate, DrugCellViewModelDelegateHandler {
    ...
    init(categoryId: Int, drugRepository: DrugRepositoryType, drugStoreRepository: DrugStoreRepositoryType, cartManager: CartManagerType) {
    ...
    func firstUploadDrugs() {
        drugRepository.getListDrugs(category: categoryId, from: 0, count: loadAtOnce)
    ...
    func loadMore() {
        if loadMoreStatus == false && allCardsLoaded == false {
            ...
            drugRepository.getListDrugs(category: categoryId, from: cellModels.value.count, count: loadAtOnce)
